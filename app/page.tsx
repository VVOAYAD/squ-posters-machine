...
function PosterPreview({ data, lang }: { data: CircularData; lang: "ar" | "en" }) {
  const isAr = lang === "ar";
...
export default function Home() {
  ...
  async function downloadPng(lang: "ar" | "en") {
    ...
    if (!node) return;
    setBusy(`PNG ${lang.toUpperCase()}`);
    try {
      // Run twice — first call sometimes misses webfonts in Chromium
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-black/30 rounded-md p-0.5 text-xs">
            <button
              ...
              className={`px-3 py-1 rounded transition-colors ${previewLang === "ar" ? "bg-[#c9a84c] text-[#500015] font-bold" : "text-white/70 hover:text-white"}`}
            >
              عربي
            </button>
            <button
              onClick={() => setPreviewLang("en")}
              className={`px-3 py-1 rounded transition-colors ${previewLang === "en" ? "bg-[#c9a84c] text-[#500015] font-bold" : "text-white/70 hover:text-white"}`}
            >
              English
            </button>
          </div>
          <button
            ...
            disabled={!!busy}
            className="flex items-center gap-1.5 bg-[#c9a84c] hover:bg-[#e8c55a] text-[#500015] font-bold text-xs px-3 py-1.5 rounded disabled:opacity-50 transition-colors"
          >
            <Download size={14} /> PNG عربي
          </button>
          <button
            ...
            className="flex items-center gap-1.5 bg-[#c9a84c] hover:bg-[#e8c55a] text-[#500015] font-bold text-xs px-3 py-1.5 rounded disabled:opacity-50 transition-colors"
          >
            <Download size={14} /> PNG English
          </button>
          <button
            onClick={downloadDocx}
            disabled={!!busy}
            className="flex items-center gap-1.5 bg-white hover:bg-[#fbfaf6] text-[#500015] font-bold text-xs px-3 py-1.5 rounded disabled:opacity-50 transition-colors"
          >
      ...
      <div className="flex-1 grid grid-cols-[440px_1fr] overflow-hidden">
        ...
        <main className="overflow-auto p-6 bg-neutral-200 relative">
          ...
          {/* Off-screen render of the other language for PNG capture */}
